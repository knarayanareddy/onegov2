...
11.9 Backend overlay contract enforcement

Goal:
- Every overlay the backend emits must have a non-empty `layer_id: str`.
- If any overlay is missing `layer_id` (or it’s blank / not a string), the backend rejects the run with error `E013_OVERLAY_CONTRACT_VIOLATION`.

File: `src/backend/utils/overlay_contract.py`
```python
...
File: `src/backend/nodes/format_scenario_output.py` (Drop-in enforcement)
```python
from src.backend.utils.overlay_contract import validate_overlays_have_layer_id
...
File: `src/backend/routers/scenario_router.py` (FastAPI / SSE error response)
```python
from fastapi import HTTPException
from src.backend.utils.overlay_contract import OverlayContractError

# SSE router error mapping:
# try:
#     ...
...
// In the switch case for map_data:
// case 'map_data': {
...
<template>
  <div class="layer-tree-panel">
    <h3>Kaartlagen</h3>
    <div v-for="layer in availableLayers" :key="layer.id" class="layer-checkbox">
      <input
        type="checkbox"
        :id="layer.id"
...
<template>
  <div class="legend-panel">
    <h3>Legenda</h3>
    <div v-for="item in legendItems" :key="item.layerId" class="legend-item">
      <h4>{{ item.label }}</h4>
      <div v-for="swatch in item.swatches" :key="swatch.meaning" class="legend-swatch">
        <span :style="{ backgroundColor: swatch.color }" class="swatch-color"></span>
...
<template>
  <div class="map-toolbar" role="toolbar" aria-label="Kaartgereedschap">
    <button
      v-for="tool in enabledTools"
      :key="tool"
      @click="$emit('tool-selected', tool)"
      :aria-label="`Activeer ${tool}`"
    >
      {{ tool }}
    </button>
  </div>
...
<template>
  <div v-if="selectedFeature" class="feature-info-panel">
    <button @click="$emit('close')">Sluiten</button>
    <h4>{{ selectedFeature.title }}</h4>
    <div v-for="(val, key) in selectedFeature.properties" :key="key">
...
<template>
  <div :id="tooltipId" role="tooltip" v-show="isVisible" class="tooltip-container">
    {{ text }}
  </div>
</template>
...
<template>
  <div v-if="isOpen" role="dialog" aria-label="Toelichting" class="popover-container">
    <button @click="$emit('close')" aria-label="Sluiten">Sluiten</button>
    <h4>{{ title }}</h4>
    <p>{{ body }}</p>
    <a :href="sourceUrl" target="_blank">📚 {{ sourceLabel }}</a>
  </div>
</template>
...
<template>
  <div class="help-icon-wrapper">
    ...
    <Tooltip :id="`tooltip-${popoverKey}`" :text="entry.short_hint_nl" :isVisible="tooltipVisible && !popoverOpen" />
    <InfoPopover
      :title="entry.title_nl"
      :body="entry.explanation_nl"
      :sourceUrl="entry.source_url"
      :sourceLabel="entry.source_label"
      :isOpen="popoverOpen"
      @close="helpStore.closePopover"
    />
  </div>
</template>


16.17 MapView.vue (conceptual rendering rule)
...
### Help System & accessibility acceptance criteria (Tooltip/Popover)
